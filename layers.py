...
class ResBlock_do_fft_fus_bench(nn.Module):
    def __init__(self, out_channel, norm='backward'):
        super(ResBlock_do_fft_fus_bench, self).__init__()
        self.main = nn.Sequential(
            BasicConv_do(out_channel, out_channel, kernel_size=3, stride=1, relu=True),
            BasicConv_do(out_channel, out_channel, kernel_size=3, stride=1, relu=False)
        )
    ...
    def forward(self, x):
        ...
        y = torch.fft.rfft2(x, norm=self.norm)
        # Z trans
        y_imag = torch.log(y.imag)
        y_real = torch.log(y.real)
        #y_real, y_imag = torch.chunk(y, 2, dim=dim)
        y = torch.complex(y_real, y_imag)
        y = torch.fft.irfft2(y, s=(H, W), norm=self.norm)
        # L function
        ...
        # Z^T trans  
        y = torch.fft.rfft2(x, norm=self.norm)
        y_imag = torch.exp(y.imag)
        y_real = torch.exp(y.real)
        y = torch.complex(y_real, y_imag)
        y = torch.fft.irfft2(y, s=(H, W), norm=self.norm)


        # Time = torch.fft.irfft2(y, s=(H, W), norm=self.norm)
        # Spatial = self.main(x)
        # Time_Spatial = torch.cat([Time, Spatial], dim = 1)
        # Fus = self.fusion(Time_Spatial)

        return self.main(x) + x + y

class ResBlock_do_fft_fus_bench_eval(nn.Module):
    def __init__(self, out_channel, norm='backward'):
        super(ResBlock_do_fft_fus_bench_eval, self).__init__()
        self.main = nn.Sequential(
            BasicConv_do_eval(out_channel, out_channel, kernel_size=3, stride=1, relu=True),
            BasicConv_do_eval(out_channel, out_channel, kernel_size=3, stride=1, relu=False)
        )
        ...
        self.fusion = nn.Sequential(
            BasicConv(out_channel * 2, out_channel, kernel_size=1, stride=1, relu=True),
            BasicConv(out_channel, out_channel, kernel_size=1, stride=1, relu=False)
        )
        self.dim = out_channel
        self.norm = norm
